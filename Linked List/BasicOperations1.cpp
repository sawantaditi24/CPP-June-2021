...
Node *Delete(Node *head, int posd)
{
    if(posd < 1 || posd > Count(head))
        return head;

    int x = -1; //to collect deleted value
    Node *p = head;
    Node *q = NULL; //tail pointer

    if(posd == 1) //deleting from begin
    {
        x = head->data;
        head = head->next;
        delete(p);
    ...
    else //deleting from last and middle
    {
        for(int i=0; i<posd-1 && p!=NULL ; i++)
        {
            q = p;
            p = p->next;
        }
    ...
    cout<<endl<<x<<" deleted from position "<<posd<<endl;
    return head;
}
...
int main()
{
    ...
    int posd;
    cout<<"The number of nodes in LL are: "<<Count(head)<<endl;
    cout<<"Want to delete (y/n)?: ";
            cin>>chd;
    if(chd=='y')
    {
        do
        {
            cout<<"Enter the position to delete from: \n 1 to delete first node, \n"<<Count(head)<<" to delete at the last, \nAny other value to delete in the middle.\nPosition: ";
            cin>>posd;
            head = Delete(head, posd);
            cout<<endl<<"Want to delete (y/n)?: ";
            cin>>chd2;
        } while(chd2!='n');
    }
